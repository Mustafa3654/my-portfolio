...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Dates: allow empty end_date => NULL
    ...
    if (empty($title) || empty($description) || empty($tech_stack)) {
        ...
        try {
            $stmt = $pdo->prepare(
                "UPDATE projects
                 SET title=?, description=?, tech_stack=?, github_link=?, live_link=?, image=?, start_date=?, end_date=?
                 WHERE id=?"
            );
            $stmt->execute([$title, $description, $tech_stack, $github_link, $live_link, $imagePath, $start_date, $end_date, $id]);
  ...
  <div class="admin-container">
    ...
    <form method="POST" enctype="multipart/form-data" class="admin-form">
      ...
      <div class="form-group">
        ...
        <input type="text" id="tech_stack" name="tech_stack" value="<?= htmlspecialchars($project['tech_stack']) ?>" required>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="url" id="github_link" name="github_link" value="<?= htmlspecialchars($project['github_link'] ?? '') ?>" placeholder="https://github.com/username/repo">
        </div>

        <div class="form-group">
          <label for="live_link"><i class="fas fa-external-link-alt"></i> Live Demo Link</label>
          <input type="url" id="live_link" name="live_link" value="<?= htmlspecialchars($project['live_link'] ?? '') ?>" placeholder="https://yourproject.com">
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <?php if (!empty($project['image'])): ?>
          <div class="current-image">
            <img src="../<?= htmlspecialchars($project['image']) ?>" alt="Current project image" style="border-radius: 8px; max-width: 200px; height: auto;">
            <span>Current image</span>
          </div>
        <?php endif; ?>
        <input type="file" name="image" accept="image/*">
        <small>Upload a new image to replace the current one. Accepted formats: JPG, PNG, GIF, WebP</small>
      </div>
